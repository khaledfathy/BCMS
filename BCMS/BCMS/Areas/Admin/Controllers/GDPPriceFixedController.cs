...
namespace BCMS.Areas.Admin.Controllers
{
    ...
    public class GDPPriceFixedController : Controller
    {
        ...
        public ActionResult Index(int? page, int id = 0)
        {
            ...
            Session["PageTitle"] = "الناتج المحلي الإجمالي  بالأسعار الثابتة";

            if (id != 0)
            {
                var model = DB.WorldGDPPriceFixeds.Where(s => s.CountryId == id).ToList().ToPagedList(page ?? 1, 10);
                return View(model);
            }
            else
            {
                var model = DB.WorldGDPPriceFixeds.ToList().ToPagedList(page ?? 1, 10);
                return View(model);
            }
        }
        ...
        [HttpPost]
        public ActionResult Create(WorldGDPPriceFixed v)
        {
            if (ModelState.IsValid)
            {

                DB.WorldGDPPriceFixeds.Add(v);
                DB.SaveChanges();
        ...
        {
            WorldGDPPriceFixed ws = DB.WorldGDPPriceFixeds.FirstOrDefault(x => x.id == id);
            if (ws != null)
            {
                ViewBag.AllYear = new SelectList(DB.Years.Select(e => new { e.YearId, e.YearN }), "YearId", "YearN");
                ViewBag.AllCountry = new SelectList(DB.Countries.Select(e => new { e.CountryId, e.CountryArName }), "CountryId", "CountryArName");
        ...
        [HttpPost]
        public ActionResult Edit(WorldGDPPriceFixed v)
        {
            if (ModelState.IsValid)
            {
        ...
        public ActionResult Delete(int id)
        {
            ...
            if (ws != null)
            {
                DB.WorldGDPPriceFixeds.Remove(ws);
                DB.SaveChanges();
