...
namespace WebTest1.Areas.Admin.Controllers
{
    ...
    public class VolumeOfExportsOfGoodsAndServicesController : Controller
    {
        ...
        public ActionResult Index(int? page,int id=0)
        {
            ...
            Session["PageTitle"] = "حجم صادرات السلع والخدمات";

            if (id != 0)
            {
                var model = DB.VolumeOfExportsOfGoodsAndServices.Where(s => s.CountryId == id).ToList().ToPagedList(page ?? 1, 10);
                return View(model);
            }
            else
            {
                var model = DB.VolumeOfExportsOfGoodsAndServices.ToList().ToPagedList(page ?? 1, 10);
                return View(model);
            }
        }
        ...
        [HttpPost]
        public ActionResult Create(VolumeOfExportsOfGoodsAndService v)
        {
            if (ModelState.IsValid)
            {

                DB.VolumeOfExportsOfGoodsAndServices.Add(v);
                DB.SaveChanges();
        ...
        {
            VolumeOfExportsOfGoodsAndService ws = DB.VolumeOfExportsOfGoodsAndServices.FirstOrDefault(x => x.ExportOfGoodsServicesID == id);
            if (ws != null)
            {
                ViewBag.AllYear = new SelectList(DB.Years.Select(e => new { e.YearId, e.YearN }), "YearId", "YearN");
                ViewBag.AllCountry = new SelectList(DB.Countries.Select(e => new { e.CountryId, e.CountryArName }), "CountryId", "CountryArName");
        ...
        public ActionResult Delete(int id)
        {
            ...
            if (ws != null)
            {
                DB.VolumeOfExportsOfGoodsAndServices.Remove(ws);
                DB.SaveChanges();
